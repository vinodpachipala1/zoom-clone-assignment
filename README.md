...
2. **Environment:** Fallback clipboard logic (`document.execCommand`) is implemented so that the "Copy Link" feature works flawlessly even when testing locally over HTTP (192.168.x.x) where the modern `navigator.clipboard` API is restricted.
...
2. Run `npm install`
3. Create a `.env.local` file with: `NEXT_PUBLIC_BACKEND_URL=http://localhost:8000`
4. Run `npm run dev`